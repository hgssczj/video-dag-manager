...
def macro_judge(
    ...
    exec_accuracy = acc_predict_model.predict(
        serv_names[0],
        {
            'reso': old_reso,
            'fps': old_fps
        },
        exec_work_condition['obj_size'], 
        exec_work_condition['obj_speed']
    )
    ...
    print('调度器内使用真实工况进行精度计算，则真实工况精度是', old_accuracy, '实际工况精度是', exec_accuracy)
