...
class KnowledgeBaseBuilder():   
    ...
    def sample_and_record(self,sample_bound,sample_range,delay_range,accuracy_range):
        ...
        # 现在有self.task_conf_names方便选取conf和flow_mapping了，可以得到任意组合。
        # 当前实现中conf_names里指定的配置是每一个服务都共享的配置，只有model需要特殊考虑。
        # 所以应该记录下服务的数目然后对应处理
        
        conf_list=[]
        for conf_name in self.task_conf_names:   #conf_names含有流水线上所有任务需要的配置参数的总和，但是不包括其所在的ip
        ...
        for conf_plan in conf_combine:
            ...
            for serv_ip_plan in serv_ip_combine:# 遍历所有配置和卸载策略组合
                ...
                for serv_cpu_plan in serv_cpu_combind:
                    ...
                    resource_limit={}
                    for i in range(0,len(self.task_conf_names)):
                        conf[self.task_conf_names[i]]=conf_plan[i]
                    for i in range(0,len(self.serv_names)):
    ...
    def sample_for_kb_sections_traverse_bayes(self,conf_sections,section_ids_to_sample,sample_bound,n_trials,bin_nums,delay_range,accuracy_range,if_continue):
        '''
         div_conf_sections={
            "reso":{
                "0":["360p","480p","540p"],
...
if __name__ == "__main__":
    ...
    if need_sparse_kb==1:
        ...
        # 从unsampled_section_ids列表中遍历各个区间，对每一个区间进行贝叶斯优化采样
        # 每一种配置下进行多少次采样（取平均值作为最终结果）
        sample_bound = 5
        # 为每一个区间进行贝叶斯优化时采样多少次（为100，就表示在这个区间上选择100个配置）
        n_trials = 100
        kb_builder.sample_for_kb_sections_traverse_bayes(conf_sections=conf_sections, # 区间划分方式
                                                   unsampled_section_ids=unsampled_section_ids, # 带采样区间编号
