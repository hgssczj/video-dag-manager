...
from kb_sec_user_clus_test import KnowledgeBaseUser
...
class KnowledgeBaseScorer():   
    ...
    def score_kb_by_runtimes(self,kb_name,refer_kb_name,runtime_filename,n_trials,optimze_goal,search_num,section_ids_sub=None):
        # 首先建立字典,选择正确字典路径        
        score_dict=dict()
        tested_num=0
        print(tested_num)
    ...
    # score_kb_by_runtimes_for_cluster
    # 用途：在建库期间，聚类函数会将已经采样的若干区间聚类为各个类，需要把每一个类当作单独的知识库来评价其性能好坏
    # 从指定runtime_filename文件中，依次读取runtime;
    # 在指定n_trials下，搜索num次，每次对所得最优解打分，求平均值
    def score_kb_by_runtimes_for_cluster(self,kb_name,refer_kb_name,runtime_filename,n_trials,optimze_goal,search_num,section_ids_sub,sec_sum,clus_id):
        # 首先建立字典,选择正确字典路径  
        print('开始评估一个聚类的性能')
        if optimze_goal==MIN_DELAY:
            # 将时延从小到大排序
            print('sec_sum=',sec_sum,'聚类id是',clus_id,'最小化时延','开始分析该聚类的性能')      
        elif optimze_goal==MAX_ACCURACY:
            print('sec_sum=',sec_sum,'聚类id是',clus_id,'最大化精度','开始分析该聚类的性能')  
        print('这个聚类内包含的区间是')
        print(section_ids_sub)
        ...
        # 最后把评估该知识库的字典写入文件，但是其路径并不是在kb_score之下
        '''
        sec_num,clus_id
        '''
        # score_dict里记录的内容形如：
        '''
        {
            "obj_n=1-obj_size=50000-obj_speed=0-bandwidth=20000-delay=0.4-accuracy=0.3-edge_cpu=0.4": 0.9888218671656602,
            "obj_n=1-obj_size=50000-obj_speed=0-bandwidth=20000-delay=0.4-accuracy=0.3-edge_cpu=0.7": 0.9888218671656602,
            "obj_n=1-obj_size=50000-obj_speed=0-bandwidth=20000-delay=0.4-accuracy=0.3-edge_cpu=1.0": 0.9888218671656602,
        }
        '''
        avg_score = sum(score_dict.values()) / len(score_dict)
        print('平均得分',avg_score)
        # 但是，我希望标准差尽可能小，同时平均得分要尽可能大，或许可以用平均分减去标准差
        ...
        for serv_name in self.serv_names:
            if len(dag_name)==0:
                ...
                dag_name+='-'+serv_name
        with open(kb_name+'/'+dag_name+'/'+'clus_score'+'/'+runtime_filename+"-"+'sec_sum='+str(sec_sum)+'-'+'clus_id='+str(clus_id)+'-'+'optimze_goal='+str(optimze_goal)+"-"+'n_trials='+str(n_trials)+"-"+'search_num='+str(search_num)+".json", 'w') as f:  
            json.dump(score_dict, f,indent=4) 
        f.close()
    ...
    def anylze_score_result(self,title_name,runtime_filename,file_dict):
        ...
        for key in file_dict.keys():
            print(key,'平均得分',sum(y_value_dict[key])/len(y_value_dict[key]))
        f.close()
    ...
    def anylze_sections_distribution(self,kb_name):
        ...
        for i in range(1,len(section_ids)):
            ...
            #sizes=[4000 for t in range(0,len(x_data))]
...
if __name__ == "__main__":
    ...
    if if_need_score==1:
        kb_name='kb_data_20-1_90i90_5clst-1'
        refer_kb_name='kb_data_90i90_no_clst-1'
        runtime_filename='runtime2-135cons'
        for optimze_goal in [MIN_DELAY]:
            for n_trials in [100]:
                kb_scorer.score_kb_by_runtimes(kb_name=kb_name,refer_kb_name=refer_kb_name,runtime_filename=runtime_filename,n_trials=n_trials,optimze_goal=optimze_goal,search_num=1)
    ...
    if if_need_draw==1:
        ...
        #'''
        file_dict={
            "sec=2 num=100":"kb_score/kb_data_3sec-1/runtime2-135cons-optimze_goal=0-n_trials=100-search_num=1.json",
            "sec=3 num=100":"kb_score/kb_data_4sec-1/runtime2-135cons-optimze_goal=0-n_trials=100-search_num=1.json",
            "sec=4 num=100":"kb_score/kb_data_5sec-1/runtime2-135cons-optimze_goal=0-n_trials=100-search_num=1.json",
            "sec=5 num=100":"kb_score/kb_data_6sec-1/runtime2-135cons-optimze_goal=0-n_trials=100-search_num=1.json",
        }
        #'''
        '''
        file_dict={
        ...
        '''
        file_dict={
            "sec=90i90 num=200":'kb_score/kb_data_90i90_no_clst-1/runtime2-135cons-optimze_goal=0-n_trials=200-search_num=1.json',
        ...
        '''
        file_dict={
            "sec=90i90            num=100":   'kb_score/kb_data_90i90_no_clst-1/runtime2-135cons-optimze_goal=0-n_trials=100-search_num=1.json',
            "sec= 4i18            num=100":   "kb_score/kb_data_5sec-1/runtime2-135cons-optimze_goal=0-n_trials=100-search_num=1.json",
            "sec=20i90-2clst      num=100":   "kb_score/kb_data_20-1_90i90_2clst-1/runtime2-135cons-optimze_goal=0-n_trials=100-search_num=1.json",
            "sec=20i90-3clst      num=100":   "kb_score/kb_data_20-1_90i90_3clst-1/runtime2-135cons-optimze_goal=0-n_trials=100-search_num=1.json",
            "sec=20i90-4clst      num=100":   "kb_score/kb_data_20-1_90i90_4clst-1/runtime2-135cons-optimze_goal=0-n_trials=100-search_num=1.json",
            "sec=20i90-5clst      num=100":   "kb_score/kb_data_20-1_90i90_5clst-1/runtime2-135cons-optimze_goal=0-n_trials=100-search_num=1.json",
            "sec=20i90-no_clst-0  num=100":   "kb_score/kb_data_20i90_no_clst-1/runtime2-135cons-optimze_goal=0-n_trials=100-search_num=1.json",
            "sec=20i90-no_clst-1  num=100":   "kb_score/kb_data_20-1_90i90_no_clst-1/runtime2-135cons-optimze_goal=0-n_trials=100-search_num=1.json",
            "sec=20i90-no_clst-2  num=100":   "kb_score/kb_data_20-1_90i90_no_clst-2/runtime2-135cons-optimze_goal=0-n_trials=100-search_num=1.json",
            "sec=20i90-no_clst-3  num=100":   "kb_score/kb_data_20-1_90i90_no_clst-3/runtime2-135cons-optimze_goal=0-n_trials=100-search_num=1.json",
            "sec=20i90-no_clst-4  num=100":   "kb_score/kb_data_20-1_90i90_no_clst-4/runtime2-135cons-optimze_goal=0-n_trials=100-search_num=1.json",
            "sec=20i90-no_clst-5  num=100":   "kb_score/kb_data_20-1_90i90_no_clst-5/runtime2-135cons-optimze_goal=0-n_trials=100-search_num=1.json",
        }
        '''
    ...
    if if_need_anylze==1:
        kb_name = 'kb_data_20-1_90i90_clst-1'
         
        kb_name = 'kb_data_20-1_90i90_2clst-1'
        kb_name = 'kb_data_20-1_90i90_3clst-1'
        kb_name = 'kb_data_20-1_90i90_4clst-1'
        kb_name = 'kb_data_20-1_90i90_5clst-1'
        kb_name = 'kb_data_5sec-1'
