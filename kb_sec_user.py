...
class  KnowledgeBaseUser():   
    ...
    def __init__(self, conf_names, serv_names, service_info_list, user_constraint, rsc_constraint, rsc_upper_bound, rsc_down_bound, work_condition, portrait_info, bandwidth_dict,kb_name):
    ...
    def get_pred_delay_in_section(self, section_id, conf, flow_mapping, resource_limit, edge_cloud_cut_choice):
        ...
        for service_info in self.service_info_list:
            # （1）加载服务对应的性能评估器
            f = open(self.kb_name+'/'+dag_name+'/'+service_info['name']+'/'+section_id+".json")  
    ...
    def update_kb(self, exe_plan, proc_delay_dict):
        ...
        for serv_name in self.serv_names:
            # 1. 加载服务已有的知识库
            kb_file_name = self.kb_name + '/' + serv_name + ".json"
